...
TEL resolves its data directory from `TEL_DIR` when set and otherwise uses
`~/.tel`. Treat that resolved directory as `<TEL_DIR>` throughout this skill.
If `<TEL_DIR>/loop-context.md` belongs to another project, **ignore the stale
file** and use `tel context --stdout` instead.
...
Write to `<TEL_DIR>/decisions/{domain}--{slug}.md`:
...
Write to `<TEL_DIR>/patterns/{slug}.md`:
...
Do not edit `<TEL_DIR>/kanban.md` directly during normal task work.
...
- Decisions (`<TEL_DIR>/decisions/`)
- Patterns (`<TEL_DIR>/patterns/`)
- Constraints (`<TEL_DIR>/constraints.md`)
- Nouns (`tel noun add`)
